...
class SimpleCalendar {
  ...
  create() {
    ...
    for (var i = 0; i < 42; i++) {
      if (i%7 === 0 || i%7 === 6) {
        ...
        days.innerHTML = days.innerHTML + '<div class="sc-item" id="sc-work"><div class="day"></div><div class="lunar-day"></div></div>';
      }
    }
    //添加下拉框数据
  ...
  updateWork() {
    ...
    var flag = 0;
    daysElement.forEach(function (v, i) {
      tmp_day = v.querySelector('.day').innerHTML;
      if (flag === 0){
        if (tmp_day === "1"){
          status = 1;
          flag = 1;
  ...
  updateEvent() {
    ...
    daysElement.forEach(function (v, i) {
      ...
      v.onclick = function () {
        ...
        var tmp_select = this.classList.valueOf();
        if (tmp_select.contains("sc-selected")) {
          this.classList.remove('sc-selected');
        }else{
  ...
  getSelectedDay() {
    ...
    days.forEach(function (v, i) {
      if (v.classList.contains('sc-selected')) {
    ...
    // var day_inner = "";
    // work_days.valueOf().forEach(function (v) {
    //   day_inner += "<div>" + v + "</div>";
    // });

    // work_day_list.innerHTML = work_days.valueOf();
